...
router.post('/api/register', registerLimiter, (req, res) => {
  try {
    ...
    if (existing) {
      ...
      // Wrap DELETE + INSERT in a transaction to prevent TOCTOU race
      const reRegister = db.getDb().transaction((email, filters) => {
        db.getDb().prepare('DELETE FROM users WHERE email = ?').run(email);
        return db.registerUser(email, filters);
      });
      result = reRegister(normalized, filters || {});
    } else {
      result = db.registerUser(normalized, filters || {});
    }
    ...
    res.status(201).json({
      message: 'Registration successful. Please check your email to verify. If you don\'t see it within a few minutes, check your spam/junk folder.',
    });
  } catch (err) {
...
router.get('/api/check-now', checkNowLimiter, async (req, res) => {
  ...
  try {
    ...
    // No recent run — run the shared pipeline (scrape → classify → store)
    // Rate limit applies only here (HTTP endpoint); cron calls runPipeline directly
    console.log('[check-now] Stale data, running full pipeline...');
    const scrapeStart = Date.now();

    const pipelineStats = await runPipeline({
      source: 'all',
      onProgress: (msg) => console.log('[check-now] ' + msg),
    });

    if (pipelineStats.errors && typeof pipelineStats.errors === 'string' && pipelineStats.errors.includes('Scrape failed')) {
    ...
    res.json({
      tenders: pipelineStats.tenders.map(formatTender),
      cached: false,
