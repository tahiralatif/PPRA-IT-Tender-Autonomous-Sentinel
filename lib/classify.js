...
function parseAiResponse(data) {
  ...
  // Treat uncertain/maybe responses as NO
  if (/^(uncertain|maybe)/i.test(normalized)) {
    return {
      relevant: false,
      reason: reply.replace(/^(UNCERTAIN|MAYBE)\s*[-—:]*\s*/i, '').trim() || 'AI was uncertain — defaulting to not relevant',
      method: 'ai',
    };
  }
...
function buildAIPrompt(tender) {
  ...
  return `You are a strict classifier for government procurement tenders.

ONLY classify as YES if the tender is DIRECTLY about IT, software, hardware, networking, cloud computing, cybersecurity, data centers, telecommunications infrastructure, or digital services for an IT/software company to deliver.

Classify as NO for:
- Physical security guards or security personnel
- Printing/photocopying equipment or services
- Office furniture or supplies
- Construction, renovation, civil works
- Vehicles, generators, solar panels
- Medical equipment or supplies
- Food, catering, uniforms
- Generic "digital transformation" consulting with no IT deliverables
...
Example: YES — This tender specifically requires procurement of cloud infrastructure services.`;
}
  ...
  if (config.ollama?.enabled) {
    try {
      ...
      if (await ollama.isAvailable()) {
        console.log(`[classify] Using local Ollama (${config.ollama.model})`);
        const result = await ollama.classifyTender(tender.title, tender.description);
        return {
      ...
      console.warn(`[classify] Ollama failed, trying Groq:`, err.message);
    }
  }
