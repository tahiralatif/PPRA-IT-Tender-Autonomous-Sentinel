...
function upsertTender(tender) {
  ...
  const existing = d
    .prepare('SELECT id, content_hash FROM tenders WHERE source = ? AND tender_ref = ?')
    .get(tender.source, tender.tenderRef);
  ...
  // Content changed — update tender (potential update/addendum)
  ...
  return { id: existing.id, isNew: false, isUpdated: true };
}
...
function upsertBatch(tenders) {
  ...
  for (const tender of tenders) {
    try {
      ...
      console.error(`[db] Error upserting ${tender.source}/${tender.tenderRef}: ${err.message}`);
      stats.errors++;
    }
  }
