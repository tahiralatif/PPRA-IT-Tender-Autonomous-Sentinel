...
function init() {
  ...
  db.exec(`
    ...
    -- AI summary columns (added via migration below)
    -- ai_summary TEXT          -- JSON: { scope, eligibility, budget, closingDate }
    -- detail_fetch_source TEXT -- 'html'|'pdf'|'fallback'|null
    -- detail_fetch_error TEXT  -- error message if fetch failed
  `);

  // ─── Migration: add AI summary columns if missing ───
  const columns = db.prepare('PRAGMA table_info(tenders)').all();
  const colNames = columns.map(c => c.name);

  if (!colNames.includes('ai_summary')) {
    db.exec(`ALTER TABLE tenders ADD COLUMN ai_summary TEXT`);
    console.log('[db] Migrated: added ai_summary column');
  }
  if (!colNames.includes('detail_fetch_source')) {
    db.exec(`ALTER TABLE tenders ADD COLUMN detail_fetch_source TEXT`);
    console.log('[db] Migrated: added detail_fetch_source column');
  }
  if (!colNames.includes('detail_fetch_error')) {
    db.exec(`ALTER TABLE tenders ADD COLUMN detail_fetch_error TEXT`);
    console.log('[db] Migrated: added detail_fetch_error column');
  }
...
function upsertTender(tender) {
  ...
  if (!existing) {
    ...
    const stmt = d.prepare(`
      INSERT INTO tenders (source, tender_ref, title, department, category, sector, closing_date, url, description, is_it_relevant, classification_method, classification_reason, content_hash, ai_summary, detail_fetch_source, detail_fetch_error)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    ...
    const result = stmt.run(
      ...
      hash,
      tender.aiSummary ? JSON.stringify(tender.aiSummary) : null,
      tender.detailFetchSource || null,
      tender.detailFetchError || null
    );
  ...
  d.prepare(`UPDATE tenders
    SET title = ?, department = ?, category = ?, sector = ?, closing_date = ?,
        ...
        detail_fetch_source = COALESCE(?, detail_fetch_source),
        detail_fetch_error = COALESCE(?, detail_fetch_error)
    WHERE id = ?
  `).run(
    tender.title || '',
    tender.department || tender.organization || '',
