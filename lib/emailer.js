...
function initTransporter() {
  ...
  if (!user || !pass) {
    console.warn('[emailer] No GMAIL_USER / GMAIL_APP_PASSWORD set — emails will not be sent');
    return null;
  }
  if (!transporter) {
...
async function sendMail({ to, subject, html, text, unsubscribeUrl, siteUrl }) {
  ...
  if (!t) {
    console.log(`[emailer] DRY RUN — would send to ${to}: ${subject}`);
  ...
  try {
    const info = await t.sendMail({
      ...
      text,
      headers: {
        'List-Unsubscribe': `<${unsubscribeUrl || (siteUrl || config.siteUrl) + '/unsubscribe'}>`,
    ...
    console.log(`[emailer] ✅ Sent to ${to}: ${subject} (id: ${info.messageId})`);
    return { sent: true, messageId: info.messageId };
  } catch (err) {
    console.error(`[emailer] ❌ Failed to send to ${to}:`, err.message);
