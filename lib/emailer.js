...
// DELIVERABILITY NOTE
// ─────────────────────────────────────────────────────────────────
// Gmail SMTP (smtp.gmail.com:587) is a shared consumer provider.
// Messages sent via Gmail are more likely to land in spam because:
//   • Gmail's shared IP reputation is shared across all Gmail users.
//   • You cannot configure SPF alignment for a non-Gmail domain.
//   • DKIM signing is limited; DMARC alignment may fail.
//
// RECOMMENDATION for production:
//   Use a dedicated ESP (Email Service Provider) that supports:
//     • SPF  — Sender Policy Framework (DNS TXT record)
//     • DKIM — DomainKeys Identified Mail (DNS CNAME + TXT)
//     • DMARC — Domain-based Message Authentication
//   Top choices:
//     1. Resend (resend.com) — simple API, good deliverability, free tier
//     2. Postmark — excellent transactional reputation
//     3. SendGrid — scalable, good free tier
//
// The Resend integration below is configured as the primary sender.
// Gmail SMTP is kept as a fallback only.
// ─────────────────────────────────────────────────────────────────
      ...
      <div style="margin-bottom:24px">
        ...
        ${newTenders.map((t, i) => `
          <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:16px;margin-bottom:12px">
            ...
            ${renderAiSummaryHtml(t.aiSummary)}
...
function buildDigestText({ userName, newTenders, updatedTenders, unsubscribeUrl, siteUrl }) {
  ...
  if (newTenders.length > 0) {
    ...
    newTenders.forEach((t, i) => {
      ...
      // AI summary in text format
      if (t.aiSummary) {
        if (t.aiSummary.scope) text += `   🎯 Scope: ${t.aiSummary.scope}\n`;
        if (t.aiSummary.eligibility) text += `   ✅ Eligibility: ${t.aiSummary.eligibility}\n`;
        if (t.aiSummary.budget && t.aiSummary.budget.toLowerCase() !== 'not specified') text += `   💰 Budget: ${t.aiSummary.budget}\n`;
      }
      text += '\n';
    });
  }
...
/**
 * Render AI summary as an HTML block for the email.
 * @param {Object|null} summary - { scope, eligibility, budget, closingDate }
 */
function renderAiSummaryHtml(summary) {
  if (!summary) return '';

  const sections = [];
  if (summary.scope) {
    sections.push(`<div style="margin-bottom:6px"><span style="font-weight:600;color:#334155">🎯 Scope:</span> <span style="color:#475569">${escapeHtml(summary.scope)}</span></div>`);
  }
  if (summary.eligibility) {
    sections.push(`<div style="margin-bottom:6px"><span style="font-weight:600;color:#334155">✅ Eligibility:</span> <span style="color:#475569">${escapeHtml(summary.eligibility)}</span></div>`);
  }
  if (summary.budget && summary.budget.toLowerCase() !== 'not specified') {
    sections.push(`<div style="margin-bottom:6px"><span style="font-weight:600;color:#334155">💰 Budget:</span> <span style="color:#475569">${escapeHtml(summary.budget)}</span></div>`);
  }

  if (sections.length === 0) return '';

  return `
    <div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:6px;padding:10px 12px;margin-top:10px;font-size:13px">
      <div style="font-weight:600;color:#1e40af;font-size:12px;margin-bottom:6px;text-transform:uppercase">AI Summary</div>
      ${sections.join('')}
    </div>
  `;
}
  ...
  try {
    const info = await t.sendMail({
      ...
      headers: {
        'List-Unsubscribe': unsub,
        'List-Unsubscribe-Post': 'List-Unsubscribe=One-Click',
        'List-Id': 'pitas-tender-alerts.jugaar.ai',
        'Feedback-ID': `pitas-digest-${Date.now()}`,
      },
    });
...
async function sendDigest(user, { newTenders, updatedTenders }) {
  ...
  const total = newTenders.length + updatedTenders.length;
  const subject = `PITAS Daily Digest — ${new Date().toLocaleDateString('en-PK')} — ${total} tender${total !== 1 ? 's' : ''}`;
...
<body style="font-family:-apple-system,BlinkMacSystemFont,sans-serif;background:#f1f5f9;padding:24px;margin:0">
  <div style="max-width:500px;margin:0 auto;background:#fff;border-radius:12px;padding:32px;text-align:center">
    ...
    <p style="color:#94a3b8;font-size:12px;margin-top:24px">If you didn't register, ignore this email. If you don't see this email in your inbox, check your spam/junk folder.</p>
  </div>
</body></html>`;

  const text = `Verify Your Email

Click the link below to activate your PITAS tender alerts subscription:
